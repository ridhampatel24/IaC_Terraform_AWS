...
module "server" {
  ...
  sg_for_server            = [module.security_group.sg_ec2_sg_ssh_http_id]
  enable_public_ip_address = true
  user_data_install_server = templatefile("./server-runner-script/server-installer.sh", {})
}


module "lb_target_group" {
  source                   = "./load-balancer-target-group"
  lb_target_group_name     = "server-lb-target-group"
  lb_target_group_port     = 80
  lb_target_group_protocol = "HTTP"
  vpc_id                   = module.networking.dev_proj_1_vpc_id
  ec2_instance_id          = module.server.server_ec2_instance_ip
}
